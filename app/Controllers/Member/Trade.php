...
class Trade extends BaseController
{
    ...
    public function getList(){
        ...
        if($List){
            ...
            if($Temp){
                ...
                $DiscountKeyValue = [];
                if($TempDiscount) $DiscountKeyValue = \App\Libraries\Tools\DatabaseTools::ListToKV($TempDiscount,"DiscountID");
            ...
            if($TempCoupon) $CouponKeyValue = \App\Libraries\Tools\DatabaseTools::ListToKV($TempCoupon,"CouponID");
        }
        //
    ...
    public function getPayment($TradeID){
        ...
        if(!$TradeData) return $this->respond(ResponseData::fail("交易編號錯誤"));
        //
        $oSubTrade = new \App\Models\Trade\SubTrade();
...
//        $oTrade->whereIn("Status",["S","A","F"]);//只能查出貨後的訂單情況
        $oTrade->where("MemberID",$LoginMemberID);
        $TradeData = $oTrade->find($TradeID);
        if(!$TradeData) return $this->respond(ResponseData::fail("交易編號錯誤或訂單尚未出貨"));
        if($TradeData["ShippingID"]!="5") return $this->respond(ResponseData::fail("運送方式不是新竹貨運"));
        if(!in_array($TradeData["Status"],["S","A","F"])) return $this->respond(ResponseData::fail("訂單尚未出貨，無法查詢貨況"));
        if(!$TradeData["ShippingCode"]) return $this->respond(ResponseData::fail("訂單尚未設定貨運編號"));
        //查詢
        $oLibHCT = new \App\Libraries\HCT\HCT();
        $ShippingInfo = $oLibHCT->getShippingInfo($TradeData["ShippingCode"]);
        $ShippingInfo = str_replace(array("\r", "\n", "\r\n", "\n\r","  "), '', $ShippingInfo);//去除無用字元
        //Res
