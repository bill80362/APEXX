...
//        $List = $oTrade->paginate($PageLimit,"",$Page);
    ...
    public function sendHCT($TradeID){
        ...
        if(!$Data) return $this->respond(ResponseData::fail("找不到該筆資料"));
        if($Data["ShippingID"]!="5") return $this->respond(ResponseData::fail("運送方式不是新竹貨運"));
        if($Data["Status"]!="T") return $this->respond(ResponseData::fail("只有[T理貨中]可以操作新竹物流接口"));
        //計算總重量
        $oSubTrade = new \App\Models\Trade\SubTrade();
    ...
    public function sendECLogistics($TradeID){
        ...
        $Data = $oTrade->find($TradeID);
        if(!$Data) return $this->respond(ResponseData::fail("找不到該筆資料"));
        if(!in_array($Data["ShippingID"],[1,2])) return $this->respond(ResponseData::fail("運送方式不是全家店到店或7-ELEVEN超商交貨便"));
        if(!in_array($Data["Status"],["T","S"])) return $this->respond(ResponseData::fail("只有[T理貨中]可以操作新竹物流接口"));
        //
